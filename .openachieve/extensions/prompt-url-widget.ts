import { DynamicBorder, type ExtensionAPI, type ExtensionContext } from "@openachieve/agent";
import { Container, Text } from "@openachieve/tui";
...
async function fetchGhMetadata(
	...
	try {
		const result = await api.exec("gh", args);
...
export default function promptUrlWidgetExtension(api: ExtensionAPI) {
	...
	const applySessionName = (ctx: ExtensionContext, match: PromptMatch, title?: string) => {
		...
		const currentName = api.getSessionName()?.trim();
		if (!currentName) {
			api.setSessionName(desiredName);
			return;
		}
	...
	api.on("before_agent_start", async (event, ctx) => {
		...
		void fetchGhMetadata(api, match.kind, match.url).then((meta) => {
