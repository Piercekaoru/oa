...
This document describes the security model and boundaries for Openachieve Agent.

In general Openachieve Agent is a coding agent that runs locally within the security boundary
...
Openachieve Agent treats the local user account and files writable by that account as inside
the same trust boundary as the Openachieve Agent process itself.  If an attacker can modify files
under the user's home directory, workspace, shell startup files, environment, or
Openachieve Agent configuration, they can generally influence Openachieve Agent or other local developer tools.
Reports that depend on such prior local write access are not security
vulnerabilities unless they demonstrate how Openachieve Agent grants that write access or crosses
an operating-system privilege boundary.

Openachieve Agent relies on users installing trustworthy extensions and loading trustworthy
skills and only to use Openachieve Agent within trusted repositories.  This is because files
...
If you believe you found a security vulnerability in Openachieve Agent or another package in
...
- Local code execution or sandboxing behavior (Openachieve Agent intentionally does not have a sandbox)
- Behavior of Openachieve Agent extensions or skills installed by the user
...
- Public internet exposure of an Openachieve Agent installation
...
- Reports requiring the ability to create, modify, delete, or replace files,
  ...
  user-controlled local state on the target machine. This includes `~/.openachieve`,
  `~/.openachieve/agent/models.json`, workspace files, `AGENTS.md`, skills, extensions,
  extension configuration, dotfiles, and files synchronized through NFS, roaming
  profiles, or dotfile managers, unless the report shows how Openachieve Agent itself grants
  that access.
- Issues caused by intentionally weakened user configuration.
- Resource/DOS claims that require trusted local input/config against Openachieve Agent.
...
For example, a report showing that malicious contents written to a trusted Openachieve Agent
configuration file cause Openachieve Agent to execute commands, load attacker-controlled tools,
...
affected and that the issue is reachable through Openachieve Agent.
