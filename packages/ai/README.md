...
TypeBox exports are re-exported from `@openachieve/ai`: `Type`, `Static`, and `TSchema`.

## Quick Start

```typescript
import { Type, getModel, stream, complete, Context, Tool, StringEnum } from '@openachieve/ai';
...
```typescript
import { Type, Tool, StringEnum } from '@openachieve/ai';
...
```typescript
import { stream, validateToolCall, Tool } from '@openachieve/ai';
...
```typescript
import { readFileSync } from 'fs';
import { getModel, complete } from '@openachieve/ai';
...
```typescript
import { getImageModel, generateImages } from '@mariozechner/ai';
...
```typescript
import { getModel, streamSimple, completeSimple } from '@openachieve/ai';
...
```typescript
import { getModel, stream } from '@openachieve/ai';
...
```typescript
import { getProviders, getModels, getModel } from '@openachieve/ai';
...
```typescript
import { Model, stream } from '@openachieve/ai';
...
Use model-level `thinkingLevelMap` to describe model-specific thinking controls. Keys are Openachieve Agent thinking levels (`off`, `minimal`, `low`, `medium`, `high`, `xhigh`). Missing keys use provider defaults, string values are sent to the provider, and `null` marks a level unsupported.
...
```typescript
import { streamAnthropic, type AnthropicOptions } from '@openachieve/ai';
...
```typescript
import { getModel, complete, Context } from '@openachieve/ai';
...
```typescript
import { Context, getModel, complete } from '@openachieve/ai';
...
- OAuth login flows are not supported in browser environments. Use the `@openachieve/ai/oauth` entry point in Node.js.
...
```typescript
import { getEnvApiKey } from '@openachieve/ai';
...
```bash
npx @openachieve/ai login              # interactive provider selection
npx @openachieve/ai login anthropic    # login to specific provider
npx @openachieve/ai list               # list available providers
```
...
The library provides login and token refresh functions via the `@openachieve/ai/oauth` entry point. Credential storage is the caller's responsibility.
...
} from '@openachieve/ai/oauth';
```

### Login Flow Example

```typescript
import { loginGitHubCopilot } from '@openachieve/ai/oauth';
...
```typescript
import { getModel, complete } from '@openachieve/ai';
import { getOAuthApiKey } from '@openachieve/ai/oauth';
...
- Add any root-level `export type` re-exports in `src/index.ts` that should remain available from `@openachieve/ai`
