 ...
 * working indicator shown while Openachieve Agent is streaming a response.
 *
 * Usage:
 *   oa --extension examples/extensions/working-indicator.ts
 *
 * Commands:
 ...
 *   /working-indicator reset     Restore Openachieve Agent's default spinner
 */

import type { ExtensionAPI, ExtensionContext, WorkingIndicatorOptions } from "@openachieve/agent";
...
function describeMode(mode: WorkingIndicatorMode): string {
	switch (mode) {
		...
		case "default":
			return "Openachieve Agent default spinner";
	}
}
