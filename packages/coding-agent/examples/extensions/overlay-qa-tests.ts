 ...
 *
 * Usage: oa --extension ./examples/extensions/overlay-qa-tests.ts
 *
 * Commands:
...
import type { ExtensionAPI, ExtensionCommandContext, Theme } from "@openachieve/agent";
import type { Component, OverlayAnchor, OverlayHandle, OverlayOptions, TUI } from "@openachieve/tui";
import { Input, matchesKey, truncateToWidth, visibleWidth } from "@openachieve/tui";
...
export default function (api: ExtensionAPI) {
	// Animation demo - proves overlays can handle real-time updates (like DOOM overlay would need)
	...
	private startProcess(): void {
		...
		this.proc = spawn("bash", [
			...
			for i in $(seq 1 100); do
				...
				DIR="/Users/nicobailon/Documents/development/openachieve-agent/packages/coding-agent/src/modes/interactive"
				...
				if [ $((i % 5)) -eq 0 ]; then
					echo -e "  \\033[32m✓ Successfully processed \${i} files in /Users/nicobailon/Documents/development/openachieve-agent\\033[0m"
				fi
				if [ $((i % 7)) -eq 0 ]; then
...
// Animation demo - proves overlays can handle real-time updates like DOOM overlay
class AnimationDemoComponent extends BaseOverlay {
	...
	render(width: number): string[] {
		...
		lines.push(border("│") + padLine(th.fg("dim", " (DOOM overlay uses same approach)")) + border("│"));
		lines.push(border("│") + padLine(``) + border("│"));
