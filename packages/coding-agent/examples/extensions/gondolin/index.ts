 ...
 *
 * Runs Openachieve Agent's built-in tools inside a local Gondolin micro-VM. The host working
 ...
 * Usage:
 *   cd /path/to/project
 *   oa -e /path/to/openachieve-agent/packages/coding-agent/examples/extensions/gondolin
 *
 * Requirements:
...
export default function (api: ExtensionAPI) {
	...
	async function startVm(ctx?: ExtensionContext): Promise<VM> {
		...
		const created = await VM.create({
			sessionLabel: `oa ${path.basename(localCwd)}`,
			vfs: {
				mounts: {
