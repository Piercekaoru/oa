 ...
 * - ~/.openachieve/agent/presets.json (global)
 * - <cwd>/.openachieve/presets.json (project-local)
 *
 ...
 *
 * Usage:
 * - `oa --preset plan` - start with plan preset
...
import { DynamicBorder, getAgentDir } from "@openachieve/agent";
import type { Api, Model } from "@openachieve/ai";
import { Container, Key, type SelectItem, SelectList, Text } from "@openachieve/tui";
...
function loadPresets(cwd: string): PresetsConfig {
	...
	const projectPath = join(cwd, ".openachieve", "presets.json");
...
export default function presetExtension(api: ExtensionAPI) {
	...
	async function showPresetSelector(ctx: ExtensionContext): Promise<void> {
		...
		if (presetNames.length === 0) {
			ctx.ui.notify(
				"No presets defined. Add presets to ~/.openachieve/agent/presets.json or .openachieve/presets.json",
				"warning",
			);
			return;
		}
