...
const SUBAGENT_INHERIT_PROJECT_CONTEXT_ENV = "OPENACHIEVE_SUBAGENT_INHERIT_PROJECT_CONTEXT";
const SUBAGENT_INHERIT_SKILLS_ENV = "OPENACHIEVE_SUBAGENT_INHERIT_SKILLS";
export const SUBAGENT_INTERCOM_SESSION_NAME_ENV = "OPENACHIEVE_SUBAGENT_INTERCOM_SESSION_NAME";
...
const SUBAGENT_ORCHESTRATION_SKILL_NAME_PATTERN = /<name>\s*oa-subagents\s*<\/name>/;
...
export function stripSubagentOrchestrationSkill(prompt: string): string {
	return prompt
		.replace(/\n{0,2}<skill\s+name=["']oa-subagents["'][^>]*>[\s\S]*?<\/skill>\n{0,2}/g, "\n\n")
...
export default function registerSubagentPromptRuntime(pi: ExtensionAPI): void {
	...
	onRuntimeEvent("context", (event: unknown) => {
		const data = event as { messages?: unknown[] };
		const messages = stripParentOnlySubagentMessages(data.messages ?? []);
		if (messages === data.messages) return undefined;
		return { messages };
	});

	onRuntimeEvent("before_agent_start", async (event: unknown) => {
		const data = event as { systemPrompt: string };
		...
		const rewritten = rewriteSubagentPrompt(data.systemPrompt, {
			inheritProjectContext: inheritProjectContext ?? true,
			inheritSkills: inheritSkills ?? true,
			fanoutChild: fanoutChild === true,
		});
		if (rewritten === data.systemPrompt) return;
