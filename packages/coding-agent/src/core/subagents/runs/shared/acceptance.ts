	...
	return (criteria ?? [])
		.map((criterion, index): ResolvedAcceptanceGate => {
			...
			return {
				...
				severity: (criterion.severity ?? "required") as "required" | "recommended",
			};
		})
...
export function resolveEffectiveAcceptance(input: {
	...
	const reviewConfig = typeof review === "object" && review !== null ? review : undefined;
	if (
		level === "reviewed" &&
		explicitLevel !== "auto" &&
		...
		review = { ...reviewConfig, required: false };
	}
	return {
		level,
...
export function formatAcceptancePrompt(acceptance: ResolvedAcceptanceConfig): string {
	...
	if (typeof acceptance.review === "object" && acceptance.review !== null) {
		lines.push(
			"",
...
export function parseAcceptanceReport(output: string): { report?: AcceptanceReport; error?: string } {
	...
	for (const body of fenced) {
		try {
			...
			const report =
				parsed && typeof parsed === "object" && "acceptance" in parsed
	...
	return {
		criteriaSatisfied: [
			...
			...input.results.map((result, index) => ({
				id: `child-${index + 1}`,
				status: (result.exitCode === 0 && result.acceptance?.status !== "rejected"
					? "satisfied"
					: "not-satisfied") as "satisfied" | "not-satisfied",
	...
	if (acceptance.level === "reviewed") {
		if (input.reviewResult) {
			...
			const optionalReview =
				typeof acceptance.review === "object" && acceptance.review !== null && acceptance.review.required === false;
			ledger.reviewResult = {
