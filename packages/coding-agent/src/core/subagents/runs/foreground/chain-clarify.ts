...
import type { Theme } from "../../compat/coding-agent.ts";
import type { Component, TUI } from "../../compat/tui.ts";
import { matchesKey, truncateToWidth, visibleWidth } from "../../compat/tui.ts";
import {
	findModelInfo,
...
function handleEditorInput(state: TextEditorState, data: string, textWidth: number): TextEditorState | null {
	...
	if (matchesKey(data, "alt+backspace")) {
		...
		return target === state.cursor
			? state
			: { ...state, buffer: state.buffer.slice(0, target) + state.buffer.slice(state.cursor), cursor: target };
	}
...
export class ChainClarifyComponent implements Component {
	...
	private handleModelSelectorInput(data: string): void {
		...
		if (matchesKey(data, "return")) {
			...
			if (selected) {
				...
				const suffix = getSupportedThinkingLevels(selectedModel).some((level) => level === requestedLevel)
					? thinkingSuffix
					: "";
	...
	private getAvailableThinkingLevels(stepIndex: number): ThinkingLevel[] {
		return getSupportedThinkingLevels(
			findModelInfo(this.getEffectiveModel(stepIndex), this.availableModels, this.preferredProvider),
		);
	}
	...
	private enterThinkingSelector(): void {
		...
		const levelIdx = levels.indexOf(suffix as (typeof levels)[number]);
	...
	private handleThinkingSelectorInput(data: string): void {
		...
		if (matchesKey(data, "up")) {
			this.thinkingSelectedIndex =
				this.thinkingSelectedIndex === 0 ? levels.length - 1 : this.thinkingSelectedIndex - 1;
			this.tui.requestRender();
			return;
		}
		...
		if (matchesKey(data, "down")) {
			this.thinkingSelectedIndex =
				this.thinkingSelectedIndex === levels.length - 1 ? 0 : this.thinkingSelectedIndex + 1;
			this.tui.requestRender();
			return;
		}
	}
	...
	private handleEditInput(data: string): void {
		...
		if (matchesKey(data, "shift+up") || matchesKey(data, "pageup" as any)) {
			const { lines: wrapped, starts } = wrapText(this.editState.buffer, textWidth);
			const cursorPos = getCursorDisplayPos(this.editState.cursor, starts);
		...
		if (matchesKey(data, "shift+down") || matchesKey(data, "pagedown" as any)) {
			const { lines: wrapped, starts } = wrapText(this.editState.buffer, textWidth);
			const cursorPos = getCursorDisplayPos(this.editState.cursor, starts);
