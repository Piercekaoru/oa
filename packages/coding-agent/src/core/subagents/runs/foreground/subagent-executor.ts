		...
		...(address
			? {
					descendantOf: {
						...
						...(address.parentStepIndex !== undefined ? { parentStepIndex: address.parentStepIndex } : {}),
					},
				}
			: {}),
	};
	...
	if (!isAsyncAvailable()) {
		return {
			content: [
				{
					type: "text",
					text: "Async mode requires upstream jiti for TypeScript execution but it could not be found. Ensure the oa-subagents package dependencies are installed.",
				},
			],
			isError: true,
			details: { mode: "single" as const, results: [] },
		...
		if (!isAsyncAvailable()) {
			return {
				content: [
					{
						type: "text",
						text: "Background mode requires upstream jiti for TypeScript execution but it could not be found. Ensure the oa-subagents package dependencies are installed.",
					},
				],
				isError: true,
				details: { mode: "chain" as const, results: [] },
...
export function createSubagentExecutor(deps: ExecutorDeps): {
	...
	const execute = async (
		...
		if (params.action) {
			...
			if (params.action === "status") {
				...
				return inspectSubagentStatus(
					paramsWithResolvedCwd as { action?: "status"; id?: string; runId?: string; dir?: string },
					{
						state: deps.state,
			...
			if (!(SUBAGENT_ACTIONS as readonly string[]).includes(params.action)) {
				return {
					content: [
						{ type: "text", text: `Unknown action: ${params.action}. Valid: ${SUBAGENT_ACTIONS.join(", ")}` },
					],
					isError: true,
			...
			return handleManagementAction(
				params.action,
				paramsWithResolvedCwd as { [key: string]: unknown; agentScope?: string },
				{ ...ctx, cwd: requestCwd },
			);
		}
		...
		const backgroundRequestedWhileClarifying =
			(hasChain || hasTasks) && requestedAsync && effectiveParams.clarify === true;
			...
			try {
				writeNestedEvent(inheritedNestedRoute, {
					...
					child: {
						...
						ownerIntercomTarget: process.env.OPENACHIEVE_SUBAGENT_INTERCOM_SESSION_NAME,
						leafIntercomTarget,
