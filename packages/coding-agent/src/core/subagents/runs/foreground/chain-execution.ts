...
import { type BehaviorOverride, ChainClarifyComponent, type ChainClarifyResult } from "./chain-clarify.ts";
...
interface ChainExecutionDetailsInput {
	...
	dynamicGroupStatuses?: Record<
		number,
		{
			status: "pending" | "running" | "completed" | "failed" | "paused" | "detached";
			error?: string;
...
async function runParallelChainTasks(input: ParallelChainRunInput): Promise<SingleResult[]> {
	...
	const parallelResults = await mapConcurrent(input.step.parallel, concurrency, async (task, taskIndex) => {
		if (aborted && failFast) {
			return {
				agent: task.agent,
				task: "(skipped)",
	...
	for (let stepIndex = 0; stepIndex < chainSteps.length; stepIndex++) {
		...
		if (isParallelStep(step)) {
			...
			try {
				...
				const parallelBehaviors = resolveParallelBehaviors(step.parallel, agents, stepIndex, chainSkills).map(
					(behavior, taskIndex) =>
						suppressProgressForReadOnlyTask(
							behavior,
				...
				for (let taskIndex = 0; taskIndex < step.parallel.length; taskIndex++) {
					...
					if (validationError)
						return buildChainExecutionErrorResult(
							validationError,
							makeDetailsInput({ currentStepIndex: stepIndex, currentFlatIndex: globalTaskIndex + taskIndex }),
						);
				}
			...
			try {
				materialized = materializeDynamicParallelStep(step, outputs, stepIndex, {
				...
				return {
					...
					details: buildChainExecutionDetails(
						makeDetailsInput({
							currentStepIndex: stepIndex,
							currentFlatIndex:
								globalTaskIndex - dynamicParallelStep.parallel.length + failures[0]!.originalIndex,
						}),
					),
				};
			}
			try {
				...
				return buildChainExecutionErrorResult(
					...
					makeDetailsInput({
						currentStepIndex: stepIndex,
						currentFlatIndex: globalTaskIndex - dynamicParallelStep.parallel.length,
					}),
				);
			}
			outputs[step.collect.as] = {
			...
			if (r.interrupted) {
				return {
					...
					details: buildChainExecutionDetails(
						makeDetailsInput({ currentStepIndex: stepIndex, currentFlatIndex: globalTaskIndex - 1 }),
					),
				};
			}
			if (r.detached) {
