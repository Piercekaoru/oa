...
import { type AvailableModelInfo, buildModelCandidates, resolveModelCandidate } from "../shared/model-fallback.ts";
import {
	nestedResultsPath,
...
function spawnRunner(cfg: object, suffix: string, cwd: string): { pid?: number; error?: string } {
	...
	proc.on("error", (error) => {
		console.error(`[oa-subagents] async spawn failed: ${error.message}`);
	});
...
const UNAVAILABLE_SUBAGENT_SKILL_ERROR = "Skills not found: oa-subagents";
...
export function executeAsyncChain(id: string, params: AsyncChainParams): AsyncExecutionResult {
	...
	const buildSeqStep = (
		...
		if (missingSkills.includes("oa-subagents"))
...
export function executeAsyncSingle(id: string, params: AsyncSingleParams): AsyncExecutionResult {
	...
	if (missingSkills.includes("oa-subagents")) return formatAsyncStartError("single", UNAVAILABLE_SUBAGENT_SKILL_ERROR);
