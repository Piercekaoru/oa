...
function runPiStreaming(
	...
	return new Promise((resolve) => {
		...
		const processStdoutLine = (line: string) => {
			...
			onChildEvent?.(event as ChildEvent);
...
async function runSingleStep(
	...
	for (let index = 0; index < candidates.length; index++) {
		...
		const completionGuard =
			run.exitCode === 0 && !run.error && !hiddenError?.hasError && step.completionGuard !== false
	...
	const acceptanceCanFailRun =
		acceptanceFailure && acceptance?.explicit && (finalResult?.exitCode ?? 1) === 0 && !finalResult?.interrupted;
...
async function runSubagent(config: SubagentRunConfig): Promise<void> {
	...
	for (let stepIndex = 0; stepIndex < steps.length; stepIndex++) {
		...
		if (isDynamicRunnerGroup(step)) {
			...
			const dynamicSteps = materialized.parallel.map((task, _itemIndex) => ({
				...step.parallel,
			...
			if (failures.length === 0) {
				try {
					...
					markDynamicGraphGroup(
						stepIndex,
						groupAcceptanceFailure ? "failed" : "completed",
