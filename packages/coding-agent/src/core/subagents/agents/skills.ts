...
const CONFIG_DIR = ".openachieve";
const SUBAGENT_ORCHESTRATION_SKILL = "oa-subagents";
...
function extractSkillPathsFromPackageRoot(
	...
	const openachieve = (pkg as { openachieve?: unknown }).openachieve;
	if (!openachieve || typeof openachieve !== "object" || Array.isArray(openachieve)) return [];
	const skills = (openachieve as { skills?: unknown }).skills;
