...
import type { AgentToolResult } from "../compat/agent-core.ts";
import { type ExtensionContext, getMarkdownTheme } from "../compat/coding-agent.ts";
import { type Component, Container, Markdown, Spacer, Text, visibleWidth } from "../compat/tui.ts";
...
function compactCurrentActivity(
	progress: Pick<
		AgentProgress,
		"currentTool" | "currentToolArgs" | "currentToolStartedAt" | "activityState" | "lastActivityAt" | "durationMs"
	>,
): string {
...
function buildMultiProgressLabel(
	...
	if (details.mode === "parallel") {
		...
		for (const progress of details.progress ?? []) {
			if (typeof progress.index === "number" && progress.index >= 0 && progress.index < totalCount) {
...
function foregroundStyleWidgetDetails(job: AsyncJobState, theme: Theme, expanded: boolean, width: number): string[] {
	if (!job.steps?.length)
		return [
			...
			...formatNestedWidgetLines(job.nestedChildren, theme, width, expanded, job.updatedAt).map(
				(line) => `  ${line}`,
			),
		];
	if (job.mode === "chain" && !job.activeParallelGroup && job.parallelGroups?.length)
...
export function renderSubagentResult(
	...
	if (d.mode === "single" && d.results.length === 1) {
		...
		const c = new Container();
		c.addChild(
			new Text(fitText(`${icon} ${theme.fg("toolTitle", theme.bold(r.agent))}${contextBadge}${progressInfo}`), 0, 0),
		);
		...
		if (isRunning && r.progress) {
			...
			c.addChild(new Text(fitText(theme.fg("accent", "Press Ctrl+O for live detail")), 0, 0));
			if (r.artifactPaths) {
				c.addChild(
					new Text(fitText(theme.fg("dim", `Artifacts: ${shortenPath(r.artifactPaths.outputPath)}`)), 0, 0),
				);
			}
	...
	for (const entry of renderEntries) {
		...
		if (rRunning && rProg) {
			if (rProg.skills?.length) {
				c.addChild(new Text(fitText(theme.fg("accent", `    skills: ${rProg.skills.join(", ")}`)), 0, 0));
			}
