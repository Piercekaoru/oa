...
export interface RunSyncOptions {
	...
	onUpdate?: (r: import("../compat/agent-core.ts").AgentToolResult<Details>) => void;
...
export const TEMP_ROOT_DIR = path.join(os.tmpdir(), `oa-subagents-${resolveTempScopeId()}`);
...
export function resolveCurrentMaxSubagentDepth(configMaxDepth?: number): number {
	return (
		normalizeMaxSubagentDepth(process.env.OPENACHIEVE_SUBAGENT_MAX_DEPTH) ??
...
export function checkSubagentDepth(configMaxDepth?: number): { blocked: boolean; depth: number; maxDepth: number } {
	const depth = Number(process.env.OPENACHIEVE_SUBAGENT_DEPTH ?? "0");
...
export function getSubagentDepthEnv(maxDepth?: number): Record<string, string> {
	const parentDepth = Number(process.env.OPENACHIEVE_SUBAGENT_DEPTH ?? "0");
	const nextDepth = Number.isFinite(parentDepth) ? parentDepth + 1 : 1;
	return {
		OPENACHIEVE_SUBAGENT_DEPTH: String(nextDepth),
		OPENACHIEVE_SUBAGENT_MAX_DEPTH: String(normalizeMaxSubagentDepth(maxDepth) ?? resolveCurrentMaxSubagentDepth()),
	};
}
