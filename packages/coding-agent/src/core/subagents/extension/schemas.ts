...
const AcceptanceOverride = Type.Unsafe({
	...
	description:
		"Optional acceptance policy. Omitted means auto-inferred; verified requires configured runtime commands.",
});
...
const DynamicExpandSchema = Type.Object(
	{
		...
		maxItems: Type.Optional(
			Type.Integer({ minimum: 0, description: "Required fanout bound unless configured globally." }),
		),
		onEmpty: Type.Optional(
			Type.String({ enum: ["skip", "fail"], description: "Empty input behavior. Defaults to skip." }),
		),
	},
	{ additionalProperties: false },
);
...
const ChainItem = Type.Object(
	...
	},
	{
		...
		allOf: [
			{
				if: { required: ["expand"] },
				// biome-ignore lint/suspicious/noThenProperty lint/complexity/useLiteralKeys: JSON Schema uses "then" as a keyword.
				["then"]: { required: ["parallel", "collect"], properties: { parallel: { type: "object" } } },
			},
			{
				if: { required: ["collect"] },
				// biome-ignore lint/suspicious/noThenProperty lint/complexity/useLiteralKeys: JSON Schema uses "then" as a keyword.
				["then"]: { required: ["expand", "parallel"], properties: { parallel: { type: "object" } } },
			},
