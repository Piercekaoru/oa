 ...
 *
 * Config file: ~/.openachieve/agent/extensions/subagent/config.json
...
import type { ExtensionAPI, ExtensionContext, ToolDefinition } from "../compat/coding-agent.ts";
import {
	Box,
	type Component,
	Container,
 ...
 * If parent session is ~/.openachieve/agent/sessions/abc123.jsonl,
 * returns ~/.openachieve/agent/sessions/abc123/ as the base.
...
• {chain_dir} - Shared directory for chain files (e.g., <tmpdir>/oa-subagents-<scope>/chain-runs/abc123/)
...
• { action: "doctor" } - read-only report for runtime paths, discovery, sessions, and intercom`,
		...
		execute(id, params, signal, onUpdate, ctx) {
			return executeSubagentCollapsed(id, params as unknown as SubagentParamsLike, signal, onUpdate, ctx);
		},
		...
		renderCall(args, theme) {
			...
			const asyncLabel =
				args.async === true && args.clarify !== true && !isParallel ? theme.fg("warning", " [async]") : "";
