> Openachieve Agent can help you use the SDK. Ask it to build an integration for your use case.

# SDK

The SDK provides programmatic access to Openachieve Agent's agent capabilities. Use it to embed Openachieve Agent in other applications, build custom interfaces, or integrate with automated workflows.
...
```typescript
import { createAgentSession, SessionManager } from "@openachieve/agent";
...
The `Agent` class (from `@openachieve/agent-core`) handles the core LLM interaction. Access it via `session.agent`.
...
const { session } = await createAgentSession({
  ...
  agentDir: "~/.openachieve/agent", // default (expands ~)
});
```

`cwd` is used by `DefaultResourceLoader` for:
- Project extensions (`.openachieve/extensions/`)
- Project skills:
  - `.openachieve/skills/`
  - `.agents/skills/` in `cwd` and ancestor directories (up to git repo root, or filesystem root when not in a repo)
- Project prompts (`.openachieve/prompts/`)
...
- Global skills:
  - `skills/` under `agentDir` (for example `~/.openachieve/agent/skills/`)
  - `~/.agents/skills/`
...
import { AuthStorage, ModelRegistry } from "@openachieve/agent";
...
// Default: uses ~/.openachieve/agent/auth.json and ~/.openachieve/agent/models.json
const authStorage = AuthStorage.create();
const modelRegistry = ModelRegistry.create(authStorage);
...
```typescript
import { createAgentSession, DefaultResourceLoader } from "@openachieve/agent";
...
The `edit` tool returns `details.diff` for Openachieve Agent's TUI display and `details.patch` as a standard unified patch for SDK consumers.

```typescript
import { createAgentSession } from "@openachieve/agent";
...
import { createAgentSession, defineTool } from "@openachieve/agent";
...
Extensions are loaded by the `ResourceLoader`. `DefaultResourceLoader` discovers extensions from `~/.openachieve/agent/extensions/`, `.openachieve/extensions/`, and settings.json extension sources.
...
```typescript
import { createEventBus, DefaultResourceLoader } from "@openachieve/agent";
...
```typescript
import { createAgentSession, SettingsManager, SessionManager } from "@openachieve/agent";
...
1. Global: `~/.openachieve/agent/settings.json`
2. Project: `<cwd>/.openachieve/settings.json`
