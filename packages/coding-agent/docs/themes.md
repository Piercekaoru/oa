> Openachieve Agent can create themes. Ask it to build one for your setup.
...
Openachieve Agent loads themes from:

- Built-in: `dark`, `light`
- Global: `~/.openachieve/agent/themes/*.json`
- Project: `.openachieve/themes/*.json`
...
On first run, Openachieve Agent detects your terminal background and defaults to `dark` or `light`.
...
```bash
mkdir -p ~/.openachieve/agent/themes
vim ~/.openachieve/agent/themes/my-theme.json
```
...
```json
{
  "$schema": "https://raw.githubusercontent.com/earendil-works/Openachieve Agent/main/packages/coding-agent/src/modes/interactive/theme/theme-schema.json",
  "name": "my-theme",
  "vars": {
    "primary": "#00aaff",
    "secondary": 242
...
**Hot reload:** When you edit the currently active custom theme file, Openachieve Agent reloads it automatically for immediate visual feedback.
...
Openachieve Agent uses 24-bit RGB colors. Most modern terminals support this (iTerm2, Kitty, WezTerm, Windows Terminal, VS Code). For older terminals with only 256-color support, Openachieve Agent falls back to the nearest approximation.
