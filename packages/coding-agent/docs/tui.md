> Openachieve Agent can create TUI components. Ask it to build one for your use case.

# TUI Components

Extensions and custom tools can render custom TUI components for interactive user interfaces. This page covers the component system and available building blocks.

**Source:** [`@openachieve/tui`](https://github.com/openachieve/openachieve-agent/tree/main/packages/tui)
...
```typescript
import { CURSOR_MARKER, type Component, type Focusable } from "@openachieve/tui";
...
The cursor remains hidden by default. This keeps the fake cursor rendering, while still positioning the hardware cursor for terminals that track IME candidate windows with hidden cursors. Some terminals require a visible hardware cursor for IME positioning; enable it with `showHardwareCursor`, `setShowHardwareCursor(true)`, or `OPENACHIEVE_HARDWARE_CURSOR=1`. The `Editor` and `Input` built-in components already implement this interface.
...
```typescript
import { Container, type Focusable, Input } from "@openachieve/tui";
...
Import from `@openachieve/tui`:

```typescript
import { Text, Box, Container, Spacer, Markdown } from "@openachieve/tui";
```
...
```typescript
import { matchesKey, Key } from "@openachieve/tui";
...
```typescript
import { visibleWidth, truncateToWidth } from "@openachieve/tui";
...
```typescript
import { getMarkdownTheme } from "@openachieve/agent";
import { Markdown } from "@openachieve/tui";
...
Set `OPENACHIEVE_TUI_WRITE_LOG` to capture the raw ANSI stream written to stdout.

```bash
OPENACHIEVE_TUI_WRITE_LOG=/tmp/tui-ansi.log npx tsx packages/tui/test/chat-simple.ts
```
...
For letting users pick from a list of options. Use `SelectList` from `@openachieve/tui` with `DynamicBorder` for framing.

```typescript
import type { ExtensionAPI } from "@openachieve/agent";
import { DynamicBorder } from "@openachieve/agent";
import { Container, type SelectItem, SelectList, Text } from "@openachieve/tui";
...
```typescript
import { BorderedLoader } from "@openachieve/agent";
...
For toggling multiple settings. Use `SettingsList` from `@openachieve/tui` with `getSettingsListTheme()`.

```typescript
import { getSettingsListTheme } from "@openachieve/agent";
import { Container, type SettingItem, SettingsList, Text } from "@openachieve/tui";
...
Customize the inline working indicator shown while Openachieve Agent is streaming a response.
...
// Restore Openachieve Agent's default spinner
ctx.ui.setWorkingIndicator();
```
...
import { matchesKey, truncateToWidth } from "@openachieve/tui";
