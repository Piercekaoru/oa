...
The extension factory can also be `async`. For dynamic model discovery, fetch and register models in the factory instead of `session_start`. Openachieve Agent waits for the factory before startup continues, so the provider is available during interactive startup and to `oa --list-models`.
...
models: [{
  ...
  // ...
  reasoning: true,
  thinkingLevelMap: {              // map Openachieve Agent levels to provider values; null hides unsupported levels
    minimal: null,
    low: null,
    medium: null,
    high: "default",
...
Credentials are persisted in `~/.openachieve/agent/auth.json`:
...
- [anthropic.ts](https://github.com/openachieve/openachieve-agent/blob/main/packages/ai/src/providers/anthropic.ts) - Anthropic Messages API
- [mistral.ts](https://github.com/openachieve/openachieve-agent/blob/main/packages/ai/src/providers/mistral.ts) - Mistral Conversations API
- [openai-completions.ts](https://github.com/openachieve/openachieve-agent/blob/main/packages/ai/src/providers/openai-completions.ts) - OpenAI Chat Completions
- [openai-responses.ts](https://github.com/openachieve/openachieve-agent/blob/main/packages/ai/src/providers/openai-responses.ts) - OpenAI Responses API
- [google.ts](https://github.com/openachieve/openachieve-agent/blob/main/packages/ai/src/providers/google.ts) - Google Generative AI
- [amazon-bedrock.ts](https://github.com/openachieve/openachieve-agent/blob/main/packages/ai/src/providers/amazon-bedrock.ts) - AWS Bedrock
...
When a request exceeds the model's context window, Openachieve Agent can recover automatically by compacting the conversation and retrying. This recovery only kicks in if Openachieve Agent recognizes the failure as an overflow.

Detection runs on the finalized assistant message:

- `stopReason === "error"`
- `errorMessage` matches one of Openachieve Agent's known overflow patterns (see [`packages/ai/src/utils/overflow.ts`](https://github.com/openachieve/openachieve-agent/blob/main/packages/ai/src/utils/overflow.ts))

If your provider returns overflow errors with a message Openachieve Agent does not recognize, normalize the error from the same extension that registers the provider. Use a `message_end` handler to rewrite the assistant message so its `errorMessage` starts with a phrase Openachieve Agent recognizes. The generic fallback `context_length_exceeded` is the safest choice.
...
`message_end` runs before Openachieve Agent tracks the assistant message for auto-compaction, so the rewritten `errorMessage` is what Openachieve Agent checks. With this in place, Openachieve Agent will:
...
- Match a provider-specific pattern, not Openachieve Agent's generic overflow patterns. Rewriting rate-limit or throttling errors (`rate limit`, `too many requests`) would falsely trigger compaction instead of Openachieve Agent's normal retry-with-backoff path.
...
Test your provider against the same test suites used by built-in providers. Copy and adapt these test files from [packages/ai/test/](https://github.com/openachieve/openachieve-agent/tree/main/packages/ai/test):
...
interface ProviderModelConfig {
  ...
  /** Maps Openachieve Agent thinking levels to provider/model-specific values; null marks a level unsupported. */
