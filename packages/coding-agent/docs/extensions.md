> Openachieve Agent can create extensions. Ask it to build one for your use case.

# Extensions

Extensions are TypeScript modules that extend Openachieve Agent's behavior. They can subscribe to lifecycle events, register custom tools callable by the LLM, add commands, and more.

> **Placement for /reload:** Put extensions in `~/.openachieve/agent/extensions/` (global) or `.openachieve/extensions/` (project-local) for auto-discovery. Use `oa -e ./path.ts` only for quick tests. Extensions in auto-discovered locations can be hot-reloaded with `/reload`.
...
Create `~/.openachieve/agent/extensions/my-extension.ts`:
...
| Location | Scope |
|----------|-------|
| `~/.openachieve/agent/extensions/*.ts` | Global (all projects) |
| `~/.openachieve/agent/extensions/*/index.ts` | Global (subdirectory) |
| `.openachieve/extensions/*.ts` | Project-local |
| `.openachieve/extensions/*/index.ts` | Project-local (subdirectory) |
...
To share extensions via npm or git as Openachieve packages, see [packages.md](packages.md).

## Available Imports

| Package | Purpose |
|---------|---------|
| `@openachieve/agent` | Extension types (`ExtensionAPI`, `ExtensionContext`, events) |
| `typebox` | Schema definitions for tool parameters |
| `@openachieve/ai` | AI utilities (`StringEnum` for Google-compatible enums) |
| `@openachieve/tui` | TUI components for custom rendering |

npm dependencies work too. Add a `package.json` next to your extension (or in a parent directory), run `npm install`, and imports from `node_modules/` are resolved automatically.

For distributed Openachieve packages installed with `oa install` (npm or git), runtime deps must be in `dependencies`. Package installation uses production installs (`npm install --omit=dev`) by default, so `devDependencies` are not available at runtime; when `npmCommand` is configured, git packages use plain `install` for compatibility with wrappers.
...
If the factory returns a `Promise`, Openachieve Agent awaits it before continuing startup. That means async initialization completes before `session_start`, before `resources_discover`, and before provider registrations queued via `api.registerProvider()` are flushed.
...
This pattern makes the fetched models available during normal startup and to `oa --list-models`.
...
After a successful switch or new-session action, Openachieve Agent emits `session_shutdown` for the old extension instance, reloads and rebinds extensions for the new session, then emits `session_start` with `reason: "new" | "resume"` and `previousSessionFile`.
...
After a successful fork or clone, Openachieve Agent emits `session_shutdown` for the old extension instance, reloads and rebinds extensions for the new session, then emits `session_start` with `reason: "fork"` and `previousSessionFile`.
Do cleanup work in `session_shutdown`, then reestablish any in-memory state in `session_start`.
...
The `systemPromptOptions` field gives extensions access to the same structured data Openachieve Agent uses to build the system prompt. This lets you inspect what Openachieve Agent has loaded — custom prompts, guidelines, tool snippets, context files, skills — without re-discovering resources or re-parsing flags. Use it when your extension needs to make deep, informed changes to the system prompt while respecting user-provided configuration.
...
This hook can rewrite provider-level system instructions or remove them entirely. Those payload-level changes are not reflected by `ctx.getSystemPrompt()`, which reports Openachieve Agent's system prompt string rather than the final serialized provider payload.
...
Before `tool_call` runs, Openachieve Agent waits for previously emitted Agent events to finish draining through `AgentSession`. This means `ctx.sessionManager` is up to date through the current assistant tool-calling message.
...
```typescript
import { isToolCallEventType } from "@openachieve/agent";
...
```typescript
import { isBashToolResult } from "@openachieve/agent";
...
```typescript
import { createLocalBashOperations } from "@openachieve/agent";
...
api.on("user_bash", (event, ctx) => {
  ...
  // Option 2: Wrap Openachieve Agent's built-in local bash backend
...
It is usually `undefined` in idle or non-turn contexts such as session events, extension commands, and shortcuts fired while Openachieve Agent is idle.
...
Returns Openachieve Agent's current system prompt string.
...
Returns the base inputs Openachieve Agent currently uses to build the system prompt.
...
- `withSession`: run post-switch work against a fresh replacement-session context. Do not use captured old `oa` / command `ctx`; see [Session replacement lifecycle and footguns](#session-replacement-lifecycle-and-footguns).
...
```typescript
import { SessionManager } from "@openachieve/agent";
...
- Captured old `oa` / old command `ctx` session-bound objects are stale after replacement and will throw if used. Use only the `ctx` passed to `withSession` for session-bound work.
...
```typescript
import { Type } from "typebox";
import { StringEnum } from "@openachieve/ai";
...
If multiple extensions register the same command name, Openachieve Agent keeps them all and assigns numeric invocation suffixes in load order, for example `/review:1` and `/review:2`.
...
```typescript
import type { AutocompleteItem } from "@openachieve/tui";
...
api.setActiveTools(["read", "bash"]); // Switch to read-only
```

`api.getAllTools()` returns `name`, `description`, `parameters`, `promptGuidelines`, and `sourceInfo`.
...
If you need to discover models from a remote endpoint, prefer an async extension factory over deferring the fetch to `session_start`. Openachieve Agent waits for the factory before startup continues, so the registered models are available immediately, including to `oa --list-models`.
...
Register tools the LLM can call via `api.registerTool()`. Tools appear in the system prompt and can have custom rendering.

Use `promptSnippet` for a short one-line entry in the `Available tools` section in the default system prompt. If omitted, custom tools are left out of that section.

Use `promptGuidelines` to add tool-specific bullets to the default system prompt `Guidelines` section. These bullets are included only while the tool is active (for example, after `api.setActiveTools([...])`).
...
```typescript
import { withFileMutationQueue } from "@openachieve/agent";
...
**Important:** Use `StringEnum` from `@openachieve/ai` for string enums. `Type.Union`/`Type.Literal` doesn't work with Google's API.

**Argument preparation:** `prepareArguments(args)` is optional. If defined, it runs before schema validation and before `execute()`. Use it to mimic an older accepted input shape when Openachieve Agent resumes an older session whose stored tool call arguments no longer match the current schema. Return the object you want validated against `parameters`. Keep the public schema strict. Do not add deprecated compatibility fields to `parameters` just to keep old resumed sessions working.
...
oa -e ./tool-override.ts
```

Alternatively, use `--no-builtin-tools` to start without any built-in tools while keeping extension tools enabled:
```bash
# No built-in tools, only extension tools
oa --no-builtin-tools -e ./my-extension.ts
```
...
- [read.ts](https://github.com/openachieve/openachieve-agent/blob/main/packages/coding-agent/src/core/tools/read.ts) - `ReadToolDetails`
- [bash.ts](https://github.com/openachieve/openachieve-agent/blob/main/packages/coding-agent/src/core/tools/bash.ts) - `BashToolDetails`
- [edit.ts](https://github.com/openachieve/openachieve-agent/blob/main/packages/coding-agent/src/core/tools/edit.ts)
- [write.ts](https://github.com/openachieve/openachieve-agent/blob/main/packages/coding-agent/src/core/tools/write.ts)
- [grep.ts](https://github.com/openachieve/openachieve-agent/blob/main/packages/coding-agent/src/core/tools/grep.ts) - `GrepToolDetails`
- [find.ts](https://github.com/openachieve/openachieve-agent/blob/main/packages/coding-agent/src/core/tools/find.ts) - `FindToolDetails`
- [ls.ts](https://github.com/openachieve/openachieve-agent/blob/main/packages/coding-agent/src/core/tools/ls.ts) - `LsToolDetails`
...
```typescript
import { createReadTool, createBashTool, type ReadOperations } from "@openachieve/agent";
...
For `user_bash`, extensions can reuse Openachieve Agent's local shell backend via `createLocalBashOperations()` instead of reimplementing local process spawning, shell resolution, and process-tree termination.
...
Tools can provide `renderCall` and `renderResult` for custom TUI display. See [tui.md](tui.md) for the full component API and [tool-execution.ts](https://github.com/openachieve/openachieve-agent/blob/main/packages/coding-agent/src/modes/interactive/components/tool-execution.ts) for how tool rows are composed.
...
```typescript
import { keyHint } from "@openachieve/agent";
...
// Terminal title
ctx.ui.setTitle("Openachieve Agent - my-project");
...
```typescript
import { Text, Component } from "@openachieve/tui";
...
```typescript
import { CustomEditor, type ExtensionAPI } from "@openachieve/agent";
import { matchesKey } from "@openachieve/tui";
...
```typescript
import { highlightCode, getLanguageFromPath } from "@openachieve/agent";
