...
**Note for Node.js/TypeScript users**: If you're building a Node.js application, consider using `AgentSession` directly from `@openachieve/agent` instead of spawning a subprocess. See [`src/core/agent-session.ts`](../src/core/agent-session.ts) for the API. For a subprocess-based TypeScript client, see [`src/modes/rpc/rpc-client.ts`](../src/modes/rpc/rpc-client.ts).
...
```json
{
  ...
  "data": {
    ...
    "cancelled": false,
    "truncated": true,
    "fullOutputPath": "/tmp/Openachieve Agent-bash-abc123.log"
  }
}
```
...
The current session name is available via `get_state` in the `sessionName` field. To set the initial name when starting RPC mode, pass `--name <name>` or `-n <name>` to the `oa --mode rpc` process.
...
```json
{
  ...
  "data": {
    "commands": [
      {"name": "session-name", "description": "Set or clear session name", "source": "extension", "path": "/home/user/.openachieve/agent/extensions/session.ts"},
      {"name": "fix-tests", "description": "Fix failing tests", "source": "prompt", "location": "project", "path": "/home/user/myproject/.openachieve/agent/prompts/fix-tests.md"},
      {"name": "skill:brave-search", "description": "Web search via Brave API", "source": "skill", "location": "user", "path": "/home/user/.openachieve/agent/skills/brave-search/SKILL.md"}
    ]
  }
}
```
...
- `location`: Where it was loaded from (optional, not present for extensions):
  - `"user"`: User-level (`~/.openachieve/agent/`)
  - `"project"`: Project-level (`./.openachieve/agent/`)
...
```json
{
  ...
  "id": "uuid-8",
  "method": "setTitle",
  "title": "Openachieve Agent - my project"
}
```
...
proc = subprocess.Popen(
    ["Openachieve Agent", "--mode", "rpc", "--no-session"],
...
const agent = spawn("Openachieve Agent", ["--mode", "rpc", "--no-session"]);
