...
describe("DefaultPackageManager", () => {
	...
	beforeEach(() => {
		previousOfflineEnv = process.env.OPENACHIEVE_OFFLINE;
	...
	afterEach(() => {
		if (previousOfflineEnv === undefined) {
			...
			process.env.OPENACHIEVE_OFFLINE = previousOfflineEnv;
		}
		vi.restoreAllMocks();
		vi.unstubAllGlobals();
...
Content`,
		...
		it("should auto-discover root markdown skills from .openachieve skill dirs", async () => {
...
Content`,
		...
		it("should resolve project paths relative to .openachieve", async () => {
			const extDir = join(tempDir, ".openachieve", "extensions");
			mkdirSync(extDir, { recursive: true });
...
Content`,
				...
				mkdirSync(join(tempDir, ".openachieve"), { recursive: true });
				...
				symlinkSync(sharedExtensionsDir, join(tempDir, ".openachieve", "extensions"), "dir");
				symlinkSync(sharedSkillsDir, join(tempDir, ".openachieve", "skills"), "dir");
				symlinkSync(sharedPromptsDir, join(tempDir, ".openachieve", "prompts"), "dir");
				symlinkSync(sharedThemesDir, join(tempDir, ".openachieve", "themes"), "dir");
		...
		it("should auto-discover project prompts with overrides", async () => {
			const promptsDir = join(tempDir, ".openachieve", "prompts");
			mkdirSync(promptsDir, { recursive: true });
		...
		it("should resolve directory with package.json openachieve.extensions in extensions setting", async () => {
			// Create a package with openachieve.extensions in package.json
			...
			// Should find the extensions declared in package.json openachieve.extensions
	...
	describe("auto-discovered skill metadata", () => {
		it("should use the agent dir as baseDir for user .openachieve/agent skills", async () => {
			const skillPath = join(agentDir, "skills", "user-openachieve", "SKILL.md");
			mkdirSync(join(agentDir, "skills", "user-openachieve"), { recursive: true });
			writeFileSync(skillPath, "---\nname: user-openachieve\ndescription: user openachieve\n---\n");
		...
		it("should use the project .openachieve dir as baseDir for project .openachieve skills", async () => {
			const projectBaseDir = join(tempDir, ".openachieve");
			const skillPath = join(projectBaseDir, "skills", "project-openachieve", "SKILL.md");
			mkdirSync(join(projectBaseDir, "skills", "project-openachieve"), { recursive: true });
			writeFileSync(skillPath, "---\nname: project-openachieve\ndescription: project openachieve\n---\n");
	...
	describe(".agents/skills auto-discovery", () => {
		...
		it("should keep ~/.agents/skills user-scoped when cwd is under home in a non-git directory", async () => {
			...
			try {
				...
				const localAgentDir = join(tempDir, ".openachieve", "agent");
		...
		it("should dedupe user skill entries when ~/.openachieve/agent/skills is a symlink to ~/.agents/skills", async () => {
			const previousHome = process.env.HOME;
			process.env.HOME = tempDir;
	...
	describe("ignore files", () => {
		...
		it("should not apply parent .gitignore to .openachieve auto-discovery", async () => {
			writeFileSync(join(tempDir, ".gitignore"), ".openachieve\n");

			const skillDir = join(tempDir, ".openachieve", "skills", "auto-skill");
			mkdirSync(skillDir, { recursive: true });
	...
	describe("resolveExtensionSources", () => {
		...
		it("should handle directories with Openachieve manifest", async () => {
		...
		it("should ignore directories with only a pi manifest", async () => {
			const pkgDir = join(tempDir, "old-openachieve-package");
			mkdirSync(pkgDir, { recursive: true });
			writeFileSync(
				join(pkgDir, "package.json"),
				JSON.stringify({
					name: "old-openachieve-package",
					pi: {
						extensions: ["./src/index.ts"],
			...
			mkdirSync(join(pkgDir, "src"), { recursive: true });
			writeFileSync(join(pkgDir, "src", "index.ts"), "export default function() {}");
			mkdirSync(join(pkgDir, "custom-skills", "my-skill"), { recursive: true });
			writeFileSync(
				join(pkgDir, "custom-skills", "my-skill", "SKILL.md"),
				"---\nname: my-skill\ndescription: Test\n---\nContent",
			);

			const result = await packageManager.resolveExtensionSources([pkgDir]);
			expect(result.extensions.some((r) => r.path === join(pkgDir, "src", "index.ts"))).toBe(false);
			expect(result.skills.some((r) => r.path === join(pkgDir, "custom-skills", "my-skill", "SKILL.md"))).toBe(
				false,
			);
		});

		it("should keep Openachieve manifest entries with leading tilde package-relative", async () => {
	...
	describe("command spawning", () => {
		it("should preserve argv entries containing spaces", () => {
			...
			const valueWithSpace = "C:\\Users\\A B\\.openachieve\\npm";
	...
	describe("npmCommand", () => {
		...
		it("should update git package dependencies with --omit=dev", async () => {
			const source = "git:github.com/user/repo";
			const targetDir = join(tempDir, ".openachieve", "git", "github.com", "user", "repo");
			mkdirSync(targetDir, { recursive: true });
			writeFileSync(join(targetDir, "package.json"), JSON.stringify({ name: "repo", version: "1.0.0" }));
		...
		it("should install user npm packages into the openachieve-managed npm root", async () => {
			settingsManager = SettingsManager.inMemory({
				npmCommand: ["pnpm"],
	...
	describe("temporary install paths", () => {
		it("should place temporary npm packages under the agent temp extension folder", () => {
			...
			expect(installPath.startsWith(join(tmpdir(), "openachieve-extensions"))).toBe(false);
	...
	describe("settings source normalization", () => {
		...
		it("should store project local packages relative to .openachieve settings base", () => {
			...
			const rel = relative(join(tempDir, ".openachieve"), projectPkgDir);
			const expected = rel.startsWith(".") ? rel : `./${rel}`;
			expect(settings.packages?.[0]).toBe(expected);
		});
	...
	describe("pattern filtering in Openachieve manifest", () => {
...
export default function(api) { api.registerTool({ name: "test", description: "test", execute: async () => helper() }); }`,
		...
		it("should respect package.json openachieve.extensions manifest in subdirectories", async () => {
	...
	describe("offline mode and network timeouts", () => {
		it("should update project npm packages using @latest when newer version is available", async () => {
			const installedPath = join(tempDir, ".openachieve", "npm", "node_modules", "example");
			...
			expect(runCommandSpy).toHaveBeenCalledWith(
				"npm",
				["install", "example@latest", "--prefix", join(tempDir, ".openachieve", "npm"), "--legacy-peer-deps"],
				undefined,
			);
		});
		...
		it("should batch npm updates per scope and run git updates in parallel while skipping pinned npm and current packages", async () => {
			...
			const projectOldPath = join(tempDir, ".openachieve", "npm", "node_modules", "project-old");
			const projectCurrentPath = join(tempDir, ".openachieve", "npm", "node_modules", "project-current");
		...
		it("should skip installing missing package sources when offline", async () => {
			process.env.OPENACHIEVE_OFFLINE = "1";
		...
		it("should skip pinned packages when checking for updates", async () => {
			const installedNpmPath = join(tempDir, ".openachieve", "npm", "node_modules", "example");
