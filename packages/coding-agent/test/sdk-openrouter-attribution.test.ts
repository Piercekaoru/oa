...
describe("createAgentSession provider attribution headers", () => {
	...
	beforeEach(() => {
		tempDir = join(tmpdir(), `openachieve-sdk-attribution-test-${Date.now()}-${Math.random().toString(36).slice(2)}`);
		cwd = join(tempDir, "project");
		agentDir = join(tempDir, "agent");
		mkdirSync(cwd, { recursive: true });
		mkdirSync(agentDir, { recursive: true });
		originalTelemetryEnv = process.env.OPENACHIEVE_TELEMETRY;
		delete process.env.OPENACHIEVE_TELEMETRY;
	});
	...
	afterEach(() => {
		if (originalTelemetryEnv === undefined) {
			...
			process.env.OPENACHIEVE_TELEMETRY = originalTelemetryEnv;
		}
	...
	it("does not apply legacy OpenRouter base URL substring attribution matching", async () => {
	...
	it("lets provider and request headers set NVIDIA NIM header keys", async () => {
		const headers = await captureHeaders(createModel("nvidia", "https://integrate.api.nvidia.com/v1"), {
			providerHeaders: {
	...
	it("adds OpenCode session headers", async () => {
		...
		expect(headers?.["x-opencode-client"]).toBe("oa");
	});
