...
import { fauxAssistantMessage, registerFauxProvider } from "@openachieve/ai";
...
describe("AgentSessionRuntime session lifecycle events", () => {
	...
	async function createRuntimeHost(extensionFactory: ExtensionFactory) {
		const tempDir = join(tmpdir(), `openachieve-runtime-events-${Date.now()}-${Math.random().toString(36).slice(2)}`);
