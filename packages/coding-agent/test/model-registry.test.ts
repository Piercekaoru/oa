...
import type { AnthropicMessagesCompat, Api, Context, Model, OpenAICompletionsCompat } from "@openachieve/ai";
import { getApiProvider } from "@openachieve/ai";
import { getOAuthProvider } from "@openachieve/ai/oauth";
...
describe("ModelRegistry", () => {
	...
	beforeEach(() => {
		tempDir = join(tmpdir(), `openachieve-test-model-registry-${Date.now()}-${Math.random().toString(36).slice(2)}`);
