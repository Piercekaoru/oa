...
import { Agent } from "@openachieve/agent-core";
import { getModel, type OAuthCredentials, type OAuthProvider } from "@openachieve/ai";
...
// ============================================================================
// OAuth API key resolution from ~/.openachieve/agent/auth.json
// ============================================================================

const AUTH_PATH = join(homedir(), ".openachieve", "agent", "auth.json");
...
/**
 * Resolve API key for a provider from ~/.openachieve/agent/auth.json
 *
...
/**
 * Check if a provider has credentials in ~/.openachieve/agent/auth.json
 */
...
/** Path to the real Openachieve Agent config directory */
export const OPENACHIEVE_AGENT_DIR = join(homedir(), ".openachieve", "agent");

/**
 * Get an AuthStorage instance backed by ~/.openachieve/agent/auth.json
...
export function createTestSession(options: TestSessionOptions = {}): TestSessionContext {
	const tempDir = join(tmpdir(), `openachieve-test-${Date.now()}-${Math.random().toString(36).slice(2)}`);
