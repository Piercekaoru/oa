...
Stateful agent with tool execution and event streaming. Built on `@openachieve/ai`.
...
```typescript
declare module "@openachieve/agent-core" {
...
```typescript
import { Agent, streamProxy } from "@openachieve/agent-core";
...
```typescript
import { agentLoop, agentLoopContinue } from "@openachieve/agent-core";
