...
const { skills: sourcedSkills } = await loadSourcedSkills<Source, SourcedSkill>(
	env,
	[
		source("project", join(env.cwd, ".openachieve/skills")),
		source("user", join(homedir(), ".openachieve/agent/skills")),
...
const { promptTemplates: sourcedPromptTemplates } = await loadSourcedPromptTemplates<Source, SourcedPromptTemplate>(
	env,
	[
		source("project", join(env.cwd, ".openachieve/prompts")),
		source("user", join(homedir(), ".openachieve/agent/prompts")),
	],
