#!/usr/bin/env bash
#
# Build Openachieve Agent binaries for all platforms locally.
...
#     oa-darwin-arm64.tar.gz
#     oa-darwin-x64.tar.gz
#     oa-linux-x64.tar.gz
#     oa-linux-arm64.tar.gz
#     oa-windows-x64.zip
#     oa-windows-arm64.zip
...
for platform in "${PLATFORMS[@]}"; do
    ...
    if [[ "$platform" == windows-* ]]; then
        bun build --compile --target=bun-$platform ./dist/bun/cli.js ./src/utils/image-resize-worker.ts --outfile "$OUTPUT_DIR/$platform/oa.exe"
    else
        bun build --compile --target=bun-$platform ./dist/bun/cli.js ./src/utils/image-resize-worker.ts --outfile "$OUTPUT_DIR/$platform/oa"
    fi
done
...
for platform in "${PLATFORMS[@]}"; do
    if [[ "$platform" == windows-* ]]; then
        # Windows (zip)
        echo "Creating oa-$platform.zip..."
        (cd "$platform" && zip -r ../oa-$platform.zip .)
    else
        # Unix platforms (tar.gz) - use wrapper directory for mise compatibility
        echo "Creating oa-$platform.tar.gz..."
        mv "$platform" oa && tar -czf oa-$platform.tar.gz oa && mv oa "$platform"
    fi
done
...
for platform in "${PLATFORMS[@]}"; do
    rm -rf "$platform"
    if [[ "$platform" == windows-* ]]; then
        mkdir -p "$platform" && (cd "$platform" && unzip -q ../oa-$platform.zip)
    else
        tar -xzf oa-$platform.tar.gz && mv oa "$platform"
    fi
done
...
for platform in "${PLATFORMS[@]}"; do
    if [[ "$platform" == windows-* ]]; then
        echo "  $OUTPUT_DIR/$platform/oa.exe"
    else
        echo "  $OUTPUT_DIR/$platform/oa"
    fi
done
