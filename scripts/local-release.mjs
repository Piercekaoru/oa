...
const packages = [
	{ directory: "packages/ai", name: "@openachieve/ai" },
	{ directory: "packages/tui", name: "@openachieve/tui" },
	{ directory: "packages/agent", name: "@openachieve/agent-core" },
	{ directory: "packages/coding-agent", name: "@openachieve/agent" },
];
...
function prepareOutputDirectory(options, repoRoot) {
	if (!options.outDir) {
		return mkdtempSync(join(tmpdir(), "openachieve-local-release-"));
	}
...
function buildBunBinaryRelease(targetDirectory, archiveDirectory) {
	...
	const archiveName = platform.startsWith("windows-") ? `oa-${platform}.zip` : `oa-${platform}.tar.gz`;
	cpSync(join(binaryBuildDirectory, archiveName), join(archiveDirectory, archiveName));
	return platform;
}

function createOaShim(installDirectory) {
	const binDirectory = join(installDirectory, "node_modules", ".bin");
	if (process.platform === "win32") {
		if (existsSync(join(binDirectory, "oa.cmd"))) {
			writeFileSync(join(installDirectory, "oa.cmd"), '@ECHO off\r\n"%~dp0node_modules\\.bin\\oa.cmd" %*\r\n');
			writeFileSync(join(installDirectory, "oa.ps1"), '& "$PSScriptRoot/node_modules/.bin/oa.ps1" @args\n');
			return;
		}
		writeFileSync(join(installDirectory, "oa.cmd"), '@ECHO off\r\n"%~dp0node_modules\\.bin\\oa.exe" %*\r\n');
		writeFileSync(join(installDirectory, "oa.ps1"), '& "$PSScriptRoot/node_modules/.bin/oa.exe" @args\n');
		return;
	}
	symlinkSync(join("node_modules", ".bin", "oa"), join(installDirectory, "oa"));
}
...
if (rootPackageJson.name !== "openachieve-agent-monorepo") {
...
if (!options.skipInstall) {
	...
	createOaShim(nodeInstallDirectory);
	...
	if (!options.skipBunInstall) {
		...
		createOaShim(bunInstallDirectory);
	}
}
...
if (!options.skipInstall) {
	...
	console.log(`  ${join(outDir, `oa-${binaryPlatform}.${String(binaryPlatform).startsWith("windows-") ? "zip" : "tar.gz"}`)}`);
	console.log("\nRun the local Bun binary release from outside the repository:");
	console.log(`  ${join(binaryDirectory, String(binaryPlatform).startsWith("windows-") ? "oa.exe" : "oa")} --help`);

	console.log("\nIsolated npm install:");
	console.log(`  ${nodeInstallDirectory}`);
	console.log("\nRun the locally packed npm CLI from outside the repository:");
	console.log(`  ${join(nodeInstallDirectory, process.platform === "win32" ? "oa.cmd" : "oa")} --help`);
	...
	if (!options.skipBunInstall) {
		...
		console.log(`  ${join(bunInstallDirectory, process.platform === "win32" ? "oa.cmd" : "oa")} --help`);
	}
}
