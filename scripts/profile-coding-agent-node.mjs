...
const agentDirEnvName = "OPENACHIEVE_CODING_AGENT_DIR";
const startupBenchmarkEnvName = "OPENACHIEVE_STARTUP_BENCHMARK";
...
Options:
  ...
  --agent-dir <dir>      Use a specific OPENACHIEVE_CODING_AGENT_DIR for the benchmark run
  --isolated-agent-dir   Use a fresh temporary agent dir instead of the normal one
  --no-offline           Do not force OPENACHIEVE_OFFLINE=1 / OPENACHIEVE_SKIP_VERSION_CHECK=1
...
function createBenchmarkEnv(options, isolatedAgentDir) {
	...
	if (options.offline) {
		env.OPENACHIEVE_OFFLINE = "1";
		env.OPENACHIEVE_SKIP_VERSION_CHECK = "1";
	}
	return env;
}
...
async function runTuiBenchmarkRun({ runtime, runIndex, measuredIndex, options, profileDir }) {
	...
	const tempRoot = options.isolatedAgentDir ? mkdtempSync(join(tmpdir(), "openachieve-startup-benchmark-")) : undefined;
